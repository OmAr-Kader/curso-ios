...
struct LecturerScreen : View {
    ...
    var body: some View {
        ...
        ZStack {
            VStack {
                ImageCacheView(state.lecturer.imageUri, contentMode: .fill).frame(width: 100, height: 100).clipShape(Circle()).onCenter()
                Text(state.lecturer.lecturerName.ifEmpty {
                    lecturerName
                }).foregroundStyle(pref.theme.textColor).padding(leading: 5, trailing: 5).font(.system(size: 14)).onCenter()
                if mode == 1 {
